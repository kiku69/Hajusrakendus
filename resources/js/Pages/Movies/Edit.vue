...
function submit() {
    form.post(route('movies.update', props.movie.id), { forceFormData: true });
}
</script>
    ...
    <div class="min-h-screen" style="background:#0d0d12; color:#e2ddd5;">
        ...
        <div class="max-w-3xl mx-auto py-10 px-4 sm:px-6 lg:px-8">
            ...
            <div style="background:#16161f; border:1px solid rgba(255,200,90,0.1); border-radius:14px; padding:2rem;">
                <h1 style="font-family:'Georgia',serif; font-size:1.5rem; color:#e2ddd5; margin-bottom:1.75rem;">Muuda filmi</h1>

                <div v-if="movie.image" style="margin-bottom:1.25rem;">
                    <img :src="movie.image.startsWith('http') ? movie.image : '/storage/' + movie.image"
                        :alt="movie.title"
                        style="height:120px; object-fit:cover; border-radius:8px; border:1px solid rgba(255,200,90,0.1);" />
                </div>
                ...
                <form @submit.prevent="submit" style="display:flex; flex-direction:column; gap:1.25rem;">
                    ...
                    <div>
                        <label style="display:block; font-size:0.72rem; color:#6b6358; letter-spacing:0.14em; text-transform:uppercase; margin-bottom:6px;">Uus pilt (valikuline)</label>
